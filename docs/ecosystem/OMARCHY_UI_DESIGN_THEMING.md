...
├── keyboard.rgb         # keyboard RGB lighting profile
├── preview.png          # thumbnail shown in the theme picker
...
├── hyprland.conf        # Hyprland compositor overrides
...
Only `colors.toml` is required for a minimum viable theme. All other files are optional overrides. Community themes often add `hyprland.conf` and `hyprlock.conf` for deeper visual integration.
...
The `[extra]` section defines semantic roles used by Omarchy templates — `accent`, `warning`, and `error`. These are separate from the 16 ANSI colors and used in Waybar, Hyprlock, and component overrides.
