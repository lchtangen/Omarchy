...
Use this after the Omarchy installer finishes and you reboot into the full Hyprland desktop.

See [OMARCHY_INSTALL_GUIDE.md](OMARCHY_INSTALL_GUIDE.md) for installation methods. The standard install command is:

```bash
curl -fsSL https://omarchy.org/install | bash
```

For the NVMe-specific install path, see [OMARCHY_NVME_INSTALL_RESUME.md](OMARCHY_NVME_INSTALL_RESUME.md).
